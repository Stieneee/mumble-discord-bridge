...
type EventDispatcher struct {
	...
	doneChan       chan struct{} // Signals when processEvents goroutine has exited
	logger         logger.Logger
	droppedEvents  uint64 // Atomic counter for dropped events
}

// NewEventDispatcher creates a new event dispatcher
func NewEventDispatcher(bridgeID string, bufferSize int, lgr logger.Logger) *EventDispatcher {
	if bufferSize <= 0 {
...
// Stop stops the event dispatcher and waits for the processEvents goroutine to exit
func (ed *EventDispatcher) Stop() {
	close(ed.stopChan)
	// Wait for processEvents goroutine to finish (it will drain remaining events)
	<-ed.doneChan
}
...
func (ed *EventDispatcher) EmitEvent(eventType BridgeEventType, data map[string]interface{}, err error) {
	...
	default:
		// Event buffer full - try drop-oldest strategy
		select {
		case <-ed.eventChan:
			// Successfully dropped oldest event
			dropped := atomic.AddUint64(&ed.droppedEvents, 1)
			// Try to add new event
			select {
			case ed.eventChan <- event:
				if ed.logger != nil {
					ed.logger.Warn("EVENTS", fmt.Sprintf("Dropped oldest event to make room (total dropped: %d)", dropped))
				}
			default:
				// Still couldn't add - very high contention
				if ed.logger != nil {
					ed.logger.Error("EVENTS", fmt.Sprintf("Event buffer full, could not add event after dropping oldest (total dropped: %d)", dropped))
				}
			}
		default:
			// Couldn't even drop oldest - channel might be empty but being read from
			dropped := atomic.AddUint64(&ed.droppedEvents, 1)
			if ed.logger != nil {
				ed.logger.Error("EVENTS", fmt.Sprintf("Event buffer contention, dropping new event (total dropped: %d)", dropped))
			}
		}
	}
}
